...
(() => {
  ...
  function evaluatePatientAI(pId) {
    ...
    if (sqi < 50) {
      ...
      if (currentHii > 0.85) {
        ...
        severity = "warning"; action = (pData.temp_grad >= 1.0) ? "Temperature Gradient elevated. Early compensatory index detected." : "Early compensatory index detected. Increase observation.";
  ...
  function updateDashboard(data, id) {
    ...
    const features = {
      HR: hr,
      HR_slope: calcBufferSlopes(patientState[id].timeBuf).hr,
      PTT: ptt,
      PTT_slope: calcBufferSlopes(patientState[id].timeBuf).ptt,
      PPG: data.ppg || 0,
      PPG_slope: 0, // (optional for now)
      Imp: bio,
      Imp_slope: calcBufferSlopes(patientState[id].timeBuf).bio,
      Temp: temp || 36.5,
      Temp_slope: 0
    };
    ...
    getAIResult(features).then(result => {
      if (result) {
        ...
        // updateUIDisplay(result.prediction, result.probability);
      }
    });
  }
  ...
  function initOverviewGrid() {
    ...
    Object.keys(patients).forEach(pId => {
      ...
      d.innerHTML = `
          <div class="pc-info">
            <div class="pc-name">${patients[pId].name}</div>
